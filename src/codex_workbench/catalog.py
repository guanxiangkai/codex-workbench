...
def definition(name,title,description,properties=None,required=None):
    ...
    if name in ('credential_details','asset_detail','knowledge_detail'):result["_meta"]={"ui":{"visibility":["app"]}}
    return result

TOOLS=[
 definition('open_workbench','工作台','打开只读工作台，不创建、修改或执行业务对象。'),
 definition('workbench_state','读取工作台','按视图读取原生记录与已登记配置；其他账户视图会受控使用已绑定凭据查询 MiniMax 官方只读用量。',{'view':{'type':'string','enum':sorted(UI_VIEWS)}}),
 definition('board_state','读取任务看板','默认汇总全部会话，可按分区、项目、会话分页读取执行轮次。',{'session_id':ID,'cursor':text(4096,True),'section_id':ID,'project_id':ID}),
 definition('task_detail','读取执行详情','读取已展示轮次的状态、耗时和关联会话，不执行或恢复任务。',{'session_id':ID,'turn_id':ID},['session_id','turn_id']),
...
TOOLS.extend([
 definition('workbench_sync','同步工作台变化','按修订返回条目差异；其他账户刷新时通过隔离消费者使用已绑定凭据查询 MiniMax 官方只读用量，失败保留快照，不返回密钥、不启动定时查询。',{'view':{'type':'string','enum':sorted(UI_VIEWS)},'session_id':ID,'cursor':text(4096,True),'section_id':ID,'project_id':ID,'scope':text(200),'query':text(300),'revision':text(64),'refresh':{'type':'boolean'}},['view']),
...
_OUTPUT_KEYS={
 ...
 'board_state':['sessions','tasks','selected_session_id','next_cursor','status'],
 'task_detail':['task','session','metrics','native_url'],'skill_detail':['skill'],
