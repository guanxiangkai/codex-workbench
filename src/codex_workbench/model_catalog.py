...
def configured_models(path):
    """读取显式模型配置，拒绝秘密字段、异常地址及未声明的模型能力。"""
    path=Path(path)
    if not path.exists():return []
    if path.stat().st_size>1024*1024:raise ValueError('模型配置超过大小限制')
    data=json.loads(path.read_text(encoding='utf-8'))
    if not isinstance(data,dict) or set(data)!={'version','models'} or data['version']!=1 or not isinstance(data['models'],list) or len(data['models'])>1000:raise ValueError('模型目录格式无效')
    result=[];seen=set()
    for item in data['models']:
        ...
        if item['id'] in seen or item['model_type'] not in TYPES:raise ValueError('模型标识或类型无效')
        ...
        seen.add(item['id']);result.append({**item,'model':item.get('model') or '', 'source':'model_catalog','validation_status':'pending','last_checked_at':None,**({'configuration_status':'missing_credential'} if not item.get('credential_id') else {})})
    return result
