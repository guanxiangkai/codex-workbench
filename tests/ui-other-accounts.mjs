...
assert(html.includes('&lt;script&gt;'));assert(!html.includes('<script>'));assert(html.includes('data-config="2.account.ref"'));assert(html.includes('使用中'));assert(html.includes('记录于'));assert(!html.includes('<summary>数据来源</summary>'));assert(!html.includes('主账户'));assert(!html.includes('默认'));assert(!html.includes('重置卡'));
