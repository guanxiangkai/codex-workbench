...
const SKILL_SCOPES={user:'个人技能',repo:'项目技能',system:'系统技能',admin:'组织技能'};
function skillsView(data=s.data,query=s.query,scope=s.kind){const scopes=[...new Set(A(data.skills).map(x=>x.scope||'unknown'))].map(id=>({id,name:SKILL_SCOPES[id]||(id==='unknown'?'其他技能':id)}));const items=newestRecords(findText(data.skills,query,['name','display_name','description','short_description']).filter(x=>!scope||(x.scope||'unknown')===scope));return `<div class="toolbar">${search()}${select('skill-scope','技能类型',options(scopes,scope,'全部类型'))}${listCount(items,'项技能')}</div><div class="skill-grid">${items.map(x=>`<article class="card skill" data-tone="${CARD_THEMES[skillTheme(x)][1]}"><div class="row">${cardSymbol(skillTheme(x))}${badge(x.enabled?'可用':'已禁用',x.enabled?'green':'muted')}</div><h2 title="${E(x.display_name||x.name)}">${E(skillTitle(x))}</h2><p>${E(x.short_description||x.description)}</p><div class="row"><span class="badge module-badge">${E(SKILL_SCOPES[x.scope]||x.scope||'其他技能')}</span><button class="soft" data-skill="${E(x.id)}">查看详情</button></div></article>`).join('')||empty('没有匹配的技能')}</div>`;}
...
function providerStyle(id){const hues={zhipu:270,bigmodel:270,deepseek:220,minimax:335,kimi:275,doubao:28,qwen:185,gemini:245,claude:15};let hash=0;for(const c of String(id||'unknown'))hash=(hash*31+c.codePointAt(0))>>>0;const hue=hues[id]??hash%360;return `--type-color:hsl(${hue} 52% 36%);--type-tint:hsl(${hue} 65% 95%)`;}
...
function otherAccountCard(a){
 ...
 return `<article class="card account other-account" data-tone="${tone}" style="${providerStyle(a.provider_id)}" aria-label="${E(a.provider_name+' · '+title)}">
 <div class="identity">${avatarMarkup(a)}<div class="grow"><h2>${E(title)}</h2><p>${E(a.provider_name)}</p></div><span class="badge provider-badge">${E(a.provider_name)}</span></div>
...
function filteredConfigurations(data,folder,kind,query){const descendants=foldersIn(A(data.folders),folder),q=String(query||'').toLocaleLowerCase();return newestRecords(A(data.entries).filter(x=>(!folder||descendants.has(x.folder_id))&&(!kind||configType(x)===kind)&&(!q||[entryTitle(x),x.label,x.name,x.id,configTypeLabel(x),folderPath(data.folders,x.folder_id),...A(x.tags)].join(' ').toLocaleLowerCase().includes(q))));}
...
function configView(){const entries=A(s.data.entries),items=filteredConfigurations(s.data,s.folder,s.kind,s.query).filter(x=>!s.configProvider||A(x.account_provider_ids).includes(s.configProvider)),kinds=s.data.facets?.kinds||[...new Map(entries.map(x=>[configType(x),{id:configType(x),name:configTypeLabel(x)}])).values()],path=folderPath(s.data.folders,s.folder);
 return `<div class="config-layout"><aside class="directories" aria-label="配置目录"><div class="directory-heading"><h2>${I('folder')}<span>配置目录</span></h2><span class="directory-total">${A(s.data.folders).length} 个目录</span></div>${folderTree()||'<p class="muted directory-empty">暂无目录</p>'}</aside><section class="config-main" aria-label="配置列表"><div class="config-list-heading"><h2>${E(path||'全部配置')}</h2>${s.folder?'<button class="config-clear" id="clear-folder">全部配置</button>':''}</div><div class="toolbar config-toolbar"><label class="search">${I('search')}<span class="sr-only">搜索配置</span><input id="query" value="${E(s.query)}" placeholder="搜索名称、目录、标签"></label>${select('kind','配置类型',options(kinds,s.kind,'全部类型'))}${A(s.data.other_account_providers).length?select('config-provider','账户平台',options(s.data.other_account_providers,s.configProvider,'全部账户平台')):''}${listCount(items,'项配置')}</div><div class="config-grid">${items.map(x=>`<article class="config-card" data-tone="${category(configType(x))[1]}"><div class="config-card-top">${typeBadge(configTypeLabel(x),configType(x))}${x.mapping_status==='stale'?badge('索引待更新','orange'):''}</div><h3 title="${E(x.label||x.name||x.id)}">${E(entryTitle(x))}</h3>${x.folder_id?`<p class="config-path">${E(folderPath(s.data.folders,x.folder_id))}</p>`:''}<div class="config-tags">${A(x.tags).map(tagBadge).join('')}${A(x.account_provider_ids).map(id=>badge(A(s.data.other_account_providers).find(p=>p.id===id)?.name||id,'blue')).join('')}</div><footer><button class="config-open" id="config-entry-${E(x.id)}" data-config="${E(x.id)}" aria-label="查看配置：${E(entryTitle(x))}">查看配置 ${I('chevron-right')}</button>${x.updated_at?`<time class="muted">${E(fmtDate(x.updated_at))}</time>`:''}</footer></article>`).join('')||empty('当前筛选下暂无配置')}</div></section></div>`;
}
...
let listSearchTimer;
function queueListSearch(event){
 s.query=event.target.value;clearTimeout(listSearchTimer);
 if(s.page==='knowledge'){s.seq++;abort?.abort();clearSnapshotPoll();}
 if(event.isComposing)return;
 listSearchTimer=setTimeout(()=>s.page==='knowledge'?load():render(),150);
}
...
async function load({cursor,session,refresh=false,poll=false}={}){
 ...
 if(['other_accounts','models','config'].includes(page))Object.assign(args,{query:s.query,kind:s.kind,provider:page==='models'?s.modelProvider:page==='config'?s.configProvider:s.accountProvider,folder:s.folder});
...
function pageChange(p){clearTimeout(listSearchTimer);clearSnapshotPoll();if(!Object.hasOwn(PAGES,p)||p===s.page)return;s.seq++;if(!bridge.embedded)history.replaceState(null,'','/'+p);s.page=p;s.modelProvider='';s.detail=null;s.data={};s.query='';s.section='';s.project='';s.folder='';s.kind='';s.accountProvider='';s.configProvider='';s.session='';s.scope='global';s.searchOpen=false;clearSecrets();load();}
...
query?.addEventListener('input',queueListSearch);
query?.addEventListener('compositionend',queueListSearch);
...
document.getElementById('knowledge-scope')?.addEventListener('change',e=>{clearTimeout(listSearchTimer);s.scope=e.target.value;s.query='';load();});
