...
function accountTitle(x){return x.display_name||x.username||x.name||x.email||'用户名未提供';}
...
const NAV={accounts:'user',other_accounts:'user',agents:'sparkles',models:'brain',config:'shield',projects:'folder',assets:'palette',knowledge:'book-open',services:'server',connections:'globe'};
...
function installBootstrap(){
 ...
 for(const item of bootstrap.views){
  ...
  const pageMatches=args.view===s.page&&(!PAGED_VIEWS.has(s.page)||args.page===undefined||Number(args.page)===s.listPage);
 ...
 s.data=current.data;s.loading=false;s.syncing=false;s.error=typeof bootstrap.error==='string'?bootstrap.error:'';if(PAGED_VIEWS.has(s.page)&&Number.isFinite(Number(current.args?.page)))s.listPage=Number(current.args.page);if(s.page==='knowledge')s.scope=s.data.selected_scope||current.args?.scope||s.scope;return true;
}
...
const PAGED_VIEWS=new Set(['accounts','other_accounts','config','agents','knowledge','models']);
let listSearchTimer;
function listPager(){const p=s.data.pagination;if(!PAGED_VIEWS.has(s.page)||!p)return '';return `<nav class="toolbar list-pagination" aria-label="列表分页"><button id="list-prev" class="soft" ${s.syncing||p.page<=1?'disabled':''}>上一页</button><span role="status">第 ${p.page} / ${Math.max(1,p.total_pages)} 页 · 共 ${p.total} 条 · 每页 ${p.page_size} 条</span><button id="list-next" class="soft" ${s.syncing||p.page>=p.total_pages?'disabled':''}>下一页</button></nav>`;}
function render(){
 ...
 root.innerHTML=`${navigation()}<main data-module="${s.page}" class="" aria-busy="${s.loading}">${pageDetail?detailView():`<div class="heading"><h1>${symbol(NAV[s.page])}${PAGES[s.page]}</h1></div>`}${s.error?`<div class="error" role="alert">${E(s.error.replaceAll('请点击刷新读取','请重试'))} <button id="retry">重试</button></div>`:''}${!pageDetail?(s.loading&&!Object.keys(s.data).length?'<div class="loading" role="status">正在读取…</div>':({accounts:accountView,other_accounts:otherAccountsView,agents:skillsView,models:modelsView,config:configView,projects:projectView,assets:assetsView,knowledge:knowledgeView,services:servicesView,connections:connectionsView}[s.page])()):''}${!pageDetail?listPager():''}</main>${searchDialog()}<div id="toast" role="status" aria-live="polite"></div>`;
 bind();hydrateAssetPreviews();document.querySelectorAll('.directories').forEach((x,i)=>{if(scrolls[i]){x.scrollTop=scrolls[i][0];x.scrollLeft=scrolls[i][1];}});
...
async function load({cursor,session,refresh=false,listPage}={}){
 ...
 if(PAGED_VIEWS.has(page)){const filters={query:s.query,kind:s.kind,provider:page==='models'?s.modelProvider:page==='config'?s.configProvider:s.accountProvider,tag:s.configTag,folder:s.folder,...(page==='knowledge'?{scope:s.scope}:{})};const scope=canonicalKey({view:page,...filters});if(scope!==s.listScope){s.listPage=1;s.listScope=scope;}Object.assign(args,filters,{page:listPage??s.listPage,page_size:20});}
 ...
 if(!cached&&PAGED_VIEWS.has(page)&&args.page===1&&!s.query&&!s.kind&&!args.provider&&!s.configTag&&!s.folder){const initial=viewCache.get(canonicalKey({view:page,...(page==='knowledge'?{scope:s.scope,query:''}:{})}));if(initial)cached=initial;}
 ...
 try{
  ...
  s.data=data;
  if(data.pagination)s.listPage=data.pagination.page;
...
function pageChange(p){clearTimeout(listSearchTimer);if(!Object.hasOwn(PAGES,p))return;s.seq++;if(!bridge.embedded)history.replaceState(null,'','/'+p);s.page=p;s.listPage=1;s.listScope='';s.modelProvider='';s.detail=null;s.data={};s.query='';s.section='';s.project='';s.folder='';s.kind='';s.configTag='';s.accountProvider='';s.configProvider='';s.session='';s.scope='global';s.assetSkill='';s.searchOpen=false;clearSecrets();load();}
...
function bind(){
 ...
 document.getElementById('list-prev')?.addEventListener('click',()=>{if(!s.syncing)load({listPage:s.data.pagination.page-1});});
 document.getElementById('list-next')?.addEventListener('click',()=>{if(!s.syncing)load({listPage:s.data.pagination.page+1});});
 ...
 document.getElementById('model-provider')?.addEventListener('change',e=>{s.modelProvider=e.target.value;load({listPage:1});});
