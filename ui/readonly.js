...
async function load({cursor,session,refresh=false,poll=false}={}){
 ...
 if(['other_accounts','models'].includes(page))args.provider=page==='models'?s.modelProvider:s.accountProvider;
 if(['other_accounts','models','config'].includes(page))Object.assign(args,{query:s.query,kind:s.kind,folder:s.folder});
...
function pageChange(p){clearTimeout(listSearchTimer);clearSnapshotPoll();if(!Object.hasOwn(PAGES,p)||p===s.page)return;s.seq++;if(!bridge.embedded)history.replaceState(null,'','/'+p);s.page=p;s.modelProvider='';s.detail=null;s.data={};s.query='';s.section='';s.project='';s.folder='';s.kind='';s.accountProvider='';s.session='';s.scope='global';s.searchOpen=false;clearSecrets();load();}
