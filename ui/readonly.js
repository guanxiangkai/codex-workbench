...
function pageChange(p){clearTimeout(listSearchTimer);if(!Object.hasOwn(PAGES,p)||p===s.page)return;s.seq++;if(!bridge.embedded)history.replaceState(null,'','/'+p);s.page=p;s.listPage=1;s.listScope='';s.modelProvider='';s.detail=null;s.data={};s.query='';s.section='';s.project='';s.folder='';s.kind='';s.configTag='';s.accountProvider='';s.configProvider='';s.session='';s.scope='global';s.assetSkill='';s.searchOpen=false;clearSecrets();load();}
...
document.addEventListener('visibilitychange',()=>{if(document.hidden){const sensitive=!!s.detail;clearSecrets();if(sensitive)render();}});
